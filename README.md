...
- **Work tokens** = non-cached input + output, counted the same way for every tool so the comparison is honest.
- Refreshes whenever I push code, plus a daily snapshot. The full day-by-day record lives in [`data/token-history.csv`](./data/token-history.csv) — so this is a real time series, not just a snapshot.
- I also make music and films. That's a different corner of my life; this one is the build log.

<sub>↑ generated by a little script that reads my local CLI logs — <a href="https://github.com/localastronaut/localastronaut/blob/main/README.md">how it works</a></sub>
